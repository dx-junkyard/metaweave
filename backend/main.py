...
## [PHASE 3 — REGEX WILDCARDING] 出力ルール
1. Qdrant の payload テキスト検索 (部分一致) に適した正規表現を生成する
2. 具体的なエンティティ名・ドメイン固有語は必ず .* ワイルドカードで置き換える
   - FORBIDDEN: \\\\[.*:Agent:Toyota\\\\]  （固有名詞 Toyota を正規表現に含めてはならない）
   - REQUIRED:  \\\\[.*:Agent:.*\\\\]      （エンティティ名部分は常に .* で汎化する）
3. 異分野検索の精度を上げるため、原則として「コア因果辺（==[...]=>）」をターゲットとした正規表現を生成する
4. ノードの :OntologyType: 部分は具体的に指定し、エンティティ名部分のみを .* にすることで
   「型は絞り込み、名前は問わない」汎用パターンを実現する
5. 複数のパターンが考えられる場合は、最も包括的な1つを選ぶ
6. 出力は JSON 形式で返す: {"query_dsl_regex": "<正規表現>", "explanation": "<日本語での簡潔な説明>"}

## Few-shot 例（エンティティ名に .* を使い汎化する手本）

入力: "限られた資源を複数の主体が奪い合う問題"
出力: {"query_dsl_regex": "\\\\[.*:Agent:.*\\\\].*==\\\\[.*compete.*\\\\]==>.*\\\\[.*:Resource:.*\\\\]", "explanation": "複数のエージェントがリソースを巡って競合するコア構造パターン（エンティティ名は問わない）"}

入力: "技術革新が既存のビジネスモデルを破壊する現象"
出力: {"query_dsl_regex": "\\\\[.*:Event:.*\\\\].*==\\\\[.*destroy.*:-\\\\]==>.*\\\\[.*:Resource:.*\\\\]", "explanation": "イベントがリソースを負の方向に変化させるコア構造（生態系・経済系どちらにもマッチ）"}

入力: "ある主体の意図的な行動が別の主体の資源獲得を促進する"
出力: {"query_dsl_regex": "\\\\[.*:Agent:.*\\\\].*==\\\\[.*enable.*:\\+\\\\]==>.*\\\\[.*:Resource:.*\\\\]", "explanation": "エージェントが別のエージェントのリソース獲得を正の方向に促進するコア構造"}

入力: "フィードバックループにより自己強化する現象"
出力: {"query_dsl_regex": "\\\\[.*:.*:.*\\\\].*==\\\\[.*cause.*:\\+\\\\]==>.*\\\\[.*:.*:.*\\\\].*==\\\\[.*reinforce.*\\\\]==>", "explanation": "正の自己強化ループ（循環因果）構造。ドメイン非依存のフィードバック検索"}
"""
